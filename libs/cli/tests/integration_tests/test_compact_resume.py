...
def _write_model_config(home_dir: Path) -> None:
    ...
    config_dir = home_dir / ".bog-agents"
    config_dir.mkdir(parents=True, exist_ok=True)
    (config_dir / "config.toml").write_text(
        """
[models.providers.itest]
class_path = "bog_agents_cli._testing_models:DeterministicIntegrationChatModel"
models = ["fake"]
""".strip()
        + "\n"
    )
...
async def _run_turn(agent, *, thread_id: str, assistant_id: str, prompt: str) -> None:
    ...
    from bog_agents_cli.textual_adapter import _build_stream_config
...
async def test_compact_resumed_thread_uses_persisted_history(
    ...
    server, resumes that thread in a fresh `BogAgentsApp`, and verifies that
    ...
    monkeypatch.setenv("BOG_AGENTS_NO_UPDATE_CHECK", "1")
    ...
    from bog_agents_cli.sessions import generate_thread_id, thread_exists
    from bog_agents_cli.widgets.messages import AppMessage, ErrorMessage

    config_path = home_dir / ".bog-agents" / "config.toml"
    ...
    try:
        ...
        async with server_session(
            ...
            channel_values = await BogAgentsApp._read_channel_values_from_checkpointer(
                thread_id
            )
