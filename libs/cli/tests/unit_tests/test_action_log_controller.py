...
def _decision(**overrides: object) -> SimpleNamespace:
    base = {"tool": "execute", "call": "rm -rf build", "decision": "reject", "rule_source": "never_allow", "risk": "high", "reason": "destructive", "judge": ""}
    base.update(overrides)
    return SimpleNamespace(**base)


def test_approvals_and_verdicts_share_one_chain(_isolated: Path) -> None:
    assert alc.record_approval_events([_decision(), _decision(decision="allow", tool="read_file")]) == 2
    ...
    kinds = [json.loads(line)["kind"] for line in logs[0].read_text(encoding="utf-8").splitlines()]
...
def test_verbs_status_verify_export_prune(_isolated: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    assert "no chains yet" in alc.run_actionlog_command("/actionlog status")
    assert "No action log to verify." == alc.run_actionlog_command("/actionlog verify")
    ...
    assert "No action log matches" in alc.run_actionlog_command("/actionlog verify nope")

    monkeypatch.setattr(alc, "signer", lambda: ((lambda payload: "sig:" + str(len(payload))), "fp-1"))
    ...
    assert verify_export(bundle, verify=lambda payload, sig: sig == "sig:" + str(len(payload))).ok
    ...
    assert "Pruned 0 chain(s)" in alc.run_actionlog_command("/actionlog prune --days 30")
    assert alc.run_actionlog_command("/actionlog prune --days") == alc.USAGE


def test_real_tracefile_signer_round_trip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sign, fingerprint = signing
    alc.record_approval_events([_decision()])
    alc.run_actionlog_command("/actionlog export")
    bundle = json.loads(next(alc.action_log_dir().glob("export-*.json")).read_text(encoding="utf-8"))
    ...
    assert verify_export(bundle, verify=lambda payload, sig: verify(material, payload, sig)).ok
