...
class TestListBenchmarkSuites:
    def test_returns_hint_when_no_suites(self, tmp_path):
        with patch("bog_agents_cli.cmd_benchmark._BENCHMARKS_DIR", tmp_path / "benchmarks"):
            with patch("bog_agents_cli.cmd_benchmark._harbor_built_in_suites", return_value=[]):
    ...
    def test_shows_builtin_tag(self, tmp_path):
        ...
        builtin_yaml.write_text("name: builtin-suite\ndescription: Built in\ntasks: []\n")

        yaml = pytest.importorskip("yaml")
        with patch("bog_agents_cli.cmd_benchmark._BENCHMARKS_DIR", benchmarks_dir):
            with patch("bog_agents_cli.cmd_benchmark._harbor_built_in_suites", return_value=[builtin_yaml]):
                result = list_benchmark_suites()
...
class TestRunBenchmark:
    def test_none_suite_lists_suites(self, tmp_path):
        with patch("bog_agents_cli.cmd_benchmark.list_benchmark_suites", return_value="LISTING") as mock_list:
    ...
    def test_suite_with_no_tasks_returns_warning(self, tmp_path):
        benchmarks_dir = tmp_path / "benchmarks"
        benchmarks_dir.mkdir()
        (benchmarks_dir / "empty.yaml").write_text("name: empty\ndescription: empty\ntasks: []\n")
    ...
    def test_runs_tasks_up_to_max(self, tmp_path):
        ...
        with patch("bog_agents_cli.cmd_benchmark._BENCHMARKS_DIR", benchmarks_dir):
            with patch("bog_agents_cli.cmd_benchmark._harbor_built_in_suites", return_value=[]):
                with patch("bog_agents_cli.cmd_benchmark._make_task_runner", return_value=lambda **_: stub_result):
    ...
    def test_shows_suite_name(self, tmp_path):
        benchmarks_dir = tmp_path / "benchmarks"
        benchmarks_dir.mkdir()
        (benchmarks_dir / "my-test.yaml").write_text("name: my-test\ndescription: x\ntasks:\n  - prompt: p1\n")
...
class TestMakeTaskRunner:
    ...
    def test_stub_runner_returns_skip_when_harbor_unavailable(self, tmp_path):
        with patch("bog_agents_cli.cmd_benchmark.logger"):
            ...
            with patch.dict("sys.modules", {"bog_agents_harbor": None, "bog_agents_harbor.backend": None}):
                runner = _make_task_runner()
        result = runner(prompt="test", expected_keywords=[], max_tokens=100, cwd=tmp_path)
...
class TestShowRecentResults:
    def test_returns_message_when_harbor_not_installed(self, tmp_path):
        with patch.dict("sys.modules", {"bog_agents_harbor": None, "bog_agents_harbor.reporter": None}):
            result = show_recent_results(cwd=tmp_path)
        assert "Harbor package not installed" in result or "not installed" in result.lower()
    ...
    def test_returns_no_trajectories_message_when_empty(self, tmp_path):
        ...
        assert "No trajectory files" in result or "not installed" in result.lower() or "Harbor" in result
