...
def test_record_validate_stale_forget(tmp_path: Path) -> None:
    ...
    assert sl.snapshot_for(tmp_path, "daytona") == (
        None,
        "no daytona snapshot recorded",
    )
    ...
    assert sl.read_lock(tmp_path)["snapshots"]["daytona"][
        "lock_hashes"
    ] == sl.current_hashes(tmp_path)
    assert sl.snapshot_for(tmp_path, "daytona")[
        0
    ] == "snap-123" and "valid" in sl.describe(tmp_path)
    ...
    assert (
        snapshot is None
        and "stale" in reason
        and "uv.lock" in reason
        and "STALE" in sl.describe(tmp_path)
    )
    (tmp_path / "package-lock.json").write_text("{}", encoding="utf-8")
    assert "package-lock.json" in sl.snapshot_for(tmp_path, "daytona")[1]

    assert sl.forget_snapshot(tmp_path, "daytona") and not sl.forget_snapshot(
        tmp_path, "daytona"
    )
