...
from bog_agents_cli.app import BogAgentsApp, _extract_model_params_flag
from bog_agents_cli.config import settings
from bog_agents_cli.model_config import ModelSpec, clear_caches
...
def mock_create_model() -> Iterator[Mock]:
    ...
    with patch(
        "bog_agents_cli.app.create_model",
...
class TestModelSwitchNoOp:
    ...
    async def test_no_message_when_switching_to_same_model(self) -> None:
        ...
        with (
            patch(
                "bog_agents_cli.model_config.has_provider_credentials",
                return_value=True,
            ),
...
class TestModelSwitchErrorHandling:
    ...
    async def test_missing_credentials_shows_error(self) -> None:
        ...
        with (
            ...
            patch(
                "bog_agents_cli.model_config.get_credential_env_var",
    ...
    async def test_save_recent_model_failure_shows_warning(self) -> None:
        ...
        with (
            ...
            patch("bog_agents_cli.app.save_recent_model", return_value=False),
        ...
        assert "~/.bog-agents/" in captured_errors[0]
    ...
    async def test_remote_agent_sets_model_override(self) -> None:
        ...
        with (
            ...
            patch(
                "bog_agents_cli.app.save_recent_model", return_value=True
            ) as mock_save,
            patch.object(AppMessage, "__init__", capture_init),
        ):
            await app._switch_model("anthropic:claude-sonnet-4-5")
    ...
    async def test_remote_agent_refreshes_model_metadata(
        ...
        with (
            ...
            patch("bog_agents_cli.app.save_recent_model", return_value=True),
        ):
...
""")
        ...
        assert any(
            "Switched to [bold]fireworks:llama-v3p1-70b[/bold]" in m for m in captured_messages
        )
...
""")
        ...
        assert any("Switched to [bold]ollama:llama3[/bold]" in m for m in captured_messages)
...
class TestModelSwitchBareModelName:
    ...
    async def test_bare_model_name_auto_detects_provider(self) -> None:
        ...
        with (
            patch("bog_agents_cli.app.detect_provider", return_value="openai"),
            patch(
                "bog_agents_cli.model_config.has_provider_credentials",
                return_value=True,
            ),
        ...
        assert any("Switched to [bold]openai:gpt-4o[/bold]" in m for m in captured_messages)
