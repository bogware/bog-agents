...
class TestListSkillsAliasDirectories:
    ...
    def test_user_agent_skills_dir_precedence(self, tmp_path: Path) -> None:
        """Test that `~/.agents/skills` overrides `~/.bog-agents/agent/skills`."""
        user_agents_dir = tmp_path / "user_agents_skills"
        user_agent_dir = tmp_path / "user_agent_skills"

        # Create same skill in both directories
        _create_skill(
            user_agents_dir / "shared-skill",
            "shared-skill",
            "From bog-agents user dir",
        )
        _create_skill(
        ...
        skills = list_skills(
            user_skills_dir=user_agents_dir,
    ...
    def test_project_agent_skills_dir_precedence(self, tmp_path: Path) -> None:
        """Test that `.agents/skills` overrides `.bog-agents/skills`."""
        project_agents_dir = tmp_path / "project_agents_skills"
        project_agent_dir = tmp_path / "project_agent_skills"

        # Create same skill in both directories
        _create_skill(
            project_agents_dir / "shared-skill",
            "shared-skill",
            "From bog-agents project dir",
        )
        _create_skill(
        ...
        skills = list_skills(
            ...
            project_skills_dir=project_agents_dir,
    ...
    def test_full_precedence_chain(self, tmp_path: Path) -> None:
        ...
        _create_skill(
            user_agents_dir / "shared-skill",
            "shared-skill",
            "From bog-agents user dir (lowest)",
        )
        _create_skill(
            user_agent_dir / "shared-skill",
            "shared-skill",
    ...
    def test_mixed_sources_with_aliases(self, tmp_path: Path) -> None:
        ...
        _create_skill(
            user_agents_dir / "skill-a",
            "skill-a",
            "Skill A from bog_agents user",
        )
        _create_skill(
        ...
        _create_skill(
            project_agents_dir / "skill-c",
            "skill-c",
            "Skill C from bog_agents project",
        )
        _create_skill(
...
class TestListSkillsBuiltIn:
    ...
    def test_real_skill_creator_ships(self) -> None:
        ...
        assert creator["compatibility"] == "designed for bog-agents-cli"
        assert "bog-agents-cli-version" in creator["metadata"]
        assert creator["metadata"]["bog-agents-cli-version"] == _cli_version
    ...
    def test_oserror_in_one_source_does_not_break_others(self, tmp_path: Path) -> None:
        ...
        original_list = __import__(
            "bog_agents.middleware.skills", fromlist=["_list_skills"]
        )._list_skills
        ...
        with patch("bog_agents_cli.skills.load.list_skills_from_backend", patched_list):
            skills = list_skills(
                built_in_skills_dir=built_in_dir,
                user_skills_dir=user_dir,
                project_skills_dir=None,
