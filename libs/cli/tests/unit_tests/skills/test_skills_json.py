...
from bog_agents_cli.skills.commands import _create, _delete, _info, _list
...
class TestSkillsListJson:
    ...
    def test_json_output_with_skills(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("bog_agents_cli.skills.load.list_skills", return_value=fake_skills),
    ...
    def test_json_output_empty(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bog_agents_cli.skills.commands.Settings") as mock_settings_cls,
            patch("bog_agents_cli.skills.load.list_skills", return_value=[]),
            patch("sys.stdout", buf),
        ):
            settings = mock_settings_cls.from_environment.return_value
            settings.get_user_skills_dir.return_value = tmp_path / "skills"
