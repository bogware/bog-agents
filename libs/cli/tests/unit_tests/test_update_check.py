...
from bog_agents_cli.update_check import (
    CACHE_TTL,
    _parse_version,
    get_latest_version,
    is_update_available,
...
def cache_file(tmp_path):
    ...
    with patch("bog_agents_cli.update_check.CACHE_FILE", path):
        yield path
...
class TestIsUpdateAvailable:
    def test_newer_available(self) -> None:
        with patch(
            "bog_agents_cli.update_check.get_latest_version", return_value="99.0.0"
        ):
    ...
    def test_current_version(self) -> None:
        with (
            patch(
                "bog_agents_cli.update_check.get_latest_version", return_value="0.0.1"
            ),
            patch("bog_agents_cli.update_check.__version__", "0.0.1"),
        ):
            available, latest = is_update_available()
    ...
    def test_ahead_of_pypi(self) -> None:
        ...
        with (
            ...
            patch("bog_agents_cli.update_check.__version__", "99.0.0"),
        ):
            available, latest = is_update_available()
    ...
    def test_fetch_failure(self) -> None:
        with patch("bog_agents_cli.update_check.get_latest_version", return_value=None):
            available, latest = is_update_available()
    ...
    def test_unparseable_pypi_version(self) -> None:
        ...
        with patch(
            "bog_agents_cli.update_check.get_latest_version",
