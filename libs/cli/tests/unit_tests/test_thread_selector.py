...
def _patch_columns(columns: dict[str, bool] | None = None) -> Any:  # noqa: ANN401
    ...
    from bog_agents_cli.model_config import THREAD_COLUMN_DEFAULTS, ThreadConfig
    ...
    def _ctx() -> Any:  # noqa: ANN401
        with (
            patch(
                "bog_agents_cli.model_config.load_thread_columns",
                return_value=dict(cols),
            ),
            patch(
                "bog_agents_cli.model_config.load_thread_sort_order",
                return_value="updated_at",
            ),
            patch(
                "bog_agents_cli.model_config.load_thread_config",
    ...
    async def test_click_selects_thread(self) -> None:
        ...
        with _patch_list_threads():
            ...
            async with app.run_test() as pilot:
                ...
                from bog_agents_cli.widgets.thread_selector import ThreadOption
...
_WEBBROWSER_OPEN = "bog_agents_cli.widgets._links.webbrowser.open"
...
class TestFetchThreadUrl:
    ...
    async def test_successful_url_updates_title(self) -> None:
        ...
        with (
            _patch_list_threads(),
            patch(
                "bog_agents_cli.widgets.thread_selector.build_langsmith_thread_url",
    ...
    async def test_timeout_leaves_title_unchanged(self) -> None:
        ...
        with (
            _patch_list_threads(),
            patch(
                "bog_agents_cli.widgets.thread_selector._URL_FETCH_TIMEOUT",
                0.01,
            ),
            patch(
                "bog_agents_cli.widgets.thread_selector.build_langsmith_thread_url",
...
class TestThreadSelectorColumnHeader:
    ...
    def test_header_contains_default_column_names(self) -> None:
        ...
        from bog_agents_cli.widgets.thread_selector import _format_header_label
...
class TestThreadSelectorLimit:
    ...
    async def test_custom_limit_is_forwarded(self) -> None:
        ...
        with (
            patch(
                "bog_agents_cli.sessions.get_thread_limit",
                return_value=5,
            ),
    ...
    async def test_checkpoint_details_are_loaded_for_initial_render(self) -> None:
        ...
        with (
            ...
            _patch_columns(),
            patch(
                "bog_agents_cli.sessions.populate_thread_checkpoint_details",
                new_callable=AsyncMock,
    ...
    async def test_cached_counts_skip_background_population(self) -> None:
        ...
        with (
            ...
            patch(
                "bog_agents_cli.sessions.apply_cached_thread_message_counts",
...
class TestThreadSelectorCheckpointDetailErrors:
    ...
    async def test_unexpected_checkpoint_detail_error_logs_warning(self) -> None:
        ...
        with (
            ...
            patch(
                "bog_agents_cli.widgets.thread_selector.logger.warning"
            ) as mock_warning,
        ):
...
class TestThreadSelectorPrefetchedRows:
    ...
    async def test_prefetched_prompt_is_preserved_during_refresh(self) -> None:
        ...
        from bog_agents_cli import sessions
...
class TestThreadSelectorDelete:
    ...
    async def test_delete_keeps_selection_on_next_thread(self) -> None:
        ...
        with (
            _patch_list_threads(),
            patch(
                "bog_agents_cli.sessions.delete_thread",
                new_callable=AsyncMock,
                return_value=None,
            ),
        ):
...
class TestThreadSelectorColumnConfig:
    ...
    def test_default_columns(self) -> None:
        ...
        from bog_agents_cli.model_config import THREAD_COLUMN_DEFAULTS
    ...
    async def test_switch_toggles_column_and_persists(self) -> None:
        ...
        with (
            ...
            _patch_columns(),
            patch(
                "bog_agents_cli.model_config.save_thread_columns",
                return_value=True,
            ) as mock_save,
        ):
            app = ThreadSelectorTestApp()
...
class TestResumeThread:
    """Tests for BogAgentsApp._resume_thread."""
    ...
    async def test_successful_switch_updates_ids(self) -> None:
        ...
        app = BogAgentsApp(thread_id="old-thread")
        app._agent = MagicMock()
        app._session_state = MagicMock()
...
class TestFetchThreadHistoryData:
    """Tests for BogAgentsApp._fetch_thread_history_data."""
    ...
    async def test_offloads_conversion_to_thread(self) -> None:
        ...
        with patch(
            "bog_agents_cli.app.asyncio.to_thread",
            new_callable=AsyncMock,
...
class TestLoadThreadHistory:
    """Tests for BogAgentsApp._load_thread_history."""
    ...
    async def test_preloaded_history_skips_fetch_and_schedules_link(self) -> None:
        ...
        app = BogAgentsApp(thread_id="tid-1")
        app._agent = MagicMock()
        fetch_history_mock = AsyncMock()
        mount_message_mock = AsyncMock()
    ...
    async def test_assistant_render_failure_does_not_abort_history_load(self) -> None:
        ...
        from bog_agents_cli.widgets.message_store import MessageData, MessageType
        from bog_agents_cli.widgets.messages import AssistantMessage
    ...
    async def test_early_return_without_thread_id_logs_debug(self) -> None:
        ...
        with patch("bog_agents_cli.app.logger.debug") as debug_mock:
...
class TestUpgradeThreadMessageLink:
    """Tests for BogAgentsApp._upgrade_thread_message_link."""
...
class TestBuildThreadMessage:
    ...
    async def test_hyperlinked_when_tracing_configured(self) -> None:
        ...
        with patch("bog_agents_cli.app.build_langsmith_thread_url", return_value=url):
            result = await app._build_thread_message("Resumed thread", "tid-123")
    ...
    async def test_fallback_on_timeout(self) -> None:
        ...
        app = BogAgentsApp()
        with patch(
            "bog_agents_cli.app.asyncio.wait_for",
...
class TestConvertMessagesToData:
    """Tests for BogAgentsApp._convert_messages_to_data."""
    ...
    def test_human_message_conversion(self) -> None:
        ...
        result = BogAgentsApp._convert_messages_to_data(msgs)
    ...
    def test_tool_call_matching(self) -> None:
        ...
        from bog_agents_cli.widgets.message_store import MessageType, ToolStatus
    ...
    def test_tool_call_error_status(self) -> None:
        ...
        from bog_agents_cli.widgets.message_store import ToolStatus
    ...
    def test_empty_messages(self) -> None:
        ...
        result = BogAgentsApp._convert_messages_to_data([])
        assert result == []
