...
from bog_agents_cli import model_config
...
from bog_agents_cli.model_config import ModelConfigError, clear_caches
...
class TestProjectAgentMdFinding:
    ...
    def test_find_agent_md_in_agents_dir(self, tmp_path: Path) -> None:
        """Test finding AGENTS.md in .bog-agents/ directory."""
        project_root = tmp_path / "project"
        project_root.mkdir()

        # Create .bog-agents/AGENTS.md
        agents_dir = project_root / ".bog-agents"
        agents_dir.mkdir()
        agent_md = agents_dir / "AGENTS.md"
    ...
    def test_find_agent_md_in_root(self, tmp_path: Path) -> None:
        ...
        # Create root-level AGENTS.md (no .bog-agents/)
    ...
    def test_both_agent_md_files_combined(self, tmp_path: Path) -> None:
        ...
        agents_md.write_text("In .bog-agents/")

        root_md = project_root / "AGENTS.md"
        root_md.write_text("In root")

        # Should return both, with .bog-agents/ first
        result = _find_project_agent_md(project_root)
        assert len(result) == 2
    ...
    def test_skips_paths_with_permission_errors(self, tmp_path: Path) -> None:
        ...
        def patched_exists(self: Path) -> bool:
            if self.name == "AGENTS.md" and ".bog-agents" in str(self):
...
class TestSettingsGetProjectAgentMdPath:
    ...
    def test_returns_existing_paths(self, tmp_path: Path) -> None:
        ...
        agents_dir = tmp_path / ".bog-agents"
        agents_dir.mkdir()
        agents_md = agents_dir / "AGENTS.md"
        agents_md.write_text("inner")
        ...
        assert result == [agents_md, root_md]
...
class TestNewlineShortcut:
    ...
    def test_returns_option_enter_on_macos(self) -> None:
        ...
        with patch("bog_agents_cli.config.sys.platform", "darwin"):
    ...
    def test_returns_ctrl_j_on_non_macos(self) -> None:
        ...
        with patch("bog_agents_cli.config.sys.platform", "linux"):
...
class TestValidateModelCapabilities:
    ...
    @patch("bog_agents_cli.config.console")
...
class TestAgentsAliasDirectories:
    ...
    def test_generic_agents_dir(self) -> None:
        """Test generic_agents_dir returns ~/.agents."""
...
""")
        ...
        with (
            ...
            caplog.at_level(logging.WARNING, logger="bog_agents_cli.config"),
        ):
            result = create_model("anthropic:claude-sonnet-4-5")
...
class TestGetLangsmithProjectName:
    ...
    def test_returns_project_from_settings(self) -> None:
        """Should prefer settings.bog_agents_langchain_project."""
        env = {
            "LANGSMITH_API_KEY": "lsv2_test",
            "LANGSMITH_TRACING": "true",
        ...
        with (
            ...
            patch("bog_agents_cli.config.settings") as mock_settings,
        ):
            mock_settings.bog_agents_langchain_project = "settings-project"
    ...
    def test_falls_back_to_env_project(self) -> None:
        ...
        with (
            ...
            mock_settings.bog_agents_langchain_project = None
...
class TestFetchLangsmithProjectUrl:
    ...
    def test_returns_none_when_lookup_times_out(self) -> None:
        ...
        with (
            patch(
                "bog_agents_cli.config._LANGSMITH_URL_LOOKUP_TIMEOUT_SECONDS",
                0.01,
            ),
...
class TestBuildLangsmithThreadUrl:
    ...
    def test_returns_url_when_configured(self) -> None:
        ...
        with (
            patch(
                "bog_agents_cli.config.get_langsmith_project_name",
        ...
        assert (
            result
            == "https://smith.langchain.com/o/org/projects/p/proj/t/thread-123?utm_source=bog-agents-cli"
        )
...
""")
        ...
        with (
            patch.object(model_config, "DEFAULT_CONFIG_PATH", config_path),
            patch(
                "bog_agents_cli.config._create_model_from_class",
...
""")
        ...
        with (
            ...
            patch(
                "bog_agents_cli.config._create_model_via_init",
                return_value=mock_instance,
            ) as mock_init,
        ):
