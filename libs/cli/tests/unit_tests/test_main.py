...
from bog_agents_cli.app import AppResult, BogAgentsApp, run_textual_app
from bog_agents_cli.config import build_langsmith_thread_url, reset_langsmith_url_cache
...
class TestRunTextualCliAsyncMcp:
    ...
    async def test_passes_server_and_mcp_kwargs_to_textual_app(self) -> None:
        ...
        with (
            ...
            patch("bog_agents_cli.model_config.save_recent_model"),
            patch("bog_agents_cli.app.run_textual_app", new=_run_textual_app_stub),
        ):
...
class TestServerCleanupLifecycle:
    ...
    async def test_deferred_server_proc_stopped_after_app_exits(self) -> None:
        ...
        async def _fake_run_async(self: BogAgentsApp) -> None:  # noqa: RUF029
...
class TestCheckOptionalTools:
    ...
    def test_returns_tool_name_when_rg_not_found(self) -> None:
        ...
        with patch("bog_agents_cli.main.shutil.which", return_value=None):
    ...
    def test_returns_empty_when_rg_found(self) -> None:
        ...
        with patch("bog_agents_cli.main.shutil.which", return_value="/usr/bin/rg"):
            missing = check_optional_tools()
