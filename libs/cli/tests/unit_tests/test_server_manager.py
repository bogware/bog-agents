...
from bog_agents_cli.server_manager import (
...
class TestStartServerAndGetAgent:
    ...
    async def test_uses_absolute_graph_and_checkpointer_refs(
        ...
        with (
            ...
            patch(
                "bog_agents_cli.server_manager.tempfile.mkdtemp",
                return_value=str(work_dir),
            ),
            patch("bog_agents_cli.server_manager.shutil.copy2"),
            patch("bog_agents_cli.server_manager._write_checkpointer"),
            patch("bog_agents_cli.server_manager._write_pyproject"),
            patch(
                "bog_agents_cli.server.generate_langgraph_json"
            ) as mock_generate_langgraph_json,
            patch("bog_agents_cli.server.ServerProcess", return_value=mock_server),
            patch("bog_agents_cli.remote_client.RemoteAgent", return_value=mock_agent),
        ):
...
class TestWritePyproject:
    ...
    def test_runtime_pyproject_relies_on_cli_dependency_only(
        ...
        """The runtime should inherit `langgraph-cli` from `bog-agents-cli`."""
        _write_pyproject(tmp_path)

        content = (tmp_path / "pyproject.toml").read_text()

        assert '"bog-agents-cli @ file://' in content
