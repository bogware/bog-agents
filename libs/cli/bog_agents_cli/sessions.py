...
async def list_threads_command(
    ...
    *,
    ...
    """CLI handler for `bog-agents threads list`.
    ...
    Args:
        ...
        sort_by: Sort field — `"updated"` or `"created"`.
            ...
            When `None`, reads from config (`~/.bog-agents/config.toml`).
    ...
    from bog_agents_cli.config import COLORS, console
    ...
    if not threads:
        ...
        console.print("[dim]Start a conversation with: bog-agents[/dim]")
        return
...
async def delete_thread_command(
    ...
    """CLI handler for: bog-agents threads delete.
