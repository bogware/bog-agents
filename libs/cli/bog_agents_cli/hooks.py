...
Loads hook configuration from `~/.bog-agents/hooks.json` and fires matching
...
Config format (`~/.bog-agents/hooks.json`):
...
Supported events (Features #25, #29, #30):
  ...
  - tool.post_call.<tool_name> — after a specific tool
...
# ============================================================================
# Tool-level hook helpers (Features #25, #29, #30)
# ============================================================================


async def dispatch_tool_pre_hook(
...
async def dispatch_tool_post_hook(
    tool_name: str, tool_args: dict[str, Any], result: str
) -> None:
    """Fire post-tool-call hooks.

    Args:
        tool_name: Name of the tool that was called.
        tool_args: Arguments that were passed.
        result: Tool result (truncated to 1000 chars).
    """
    payload = {
        "tool_name": tool_name,
        "tool_args": tool_args,
...
async def dispatch_file_hook(event: str, file_path: str, **kwargs: Any) -> None:
    """Fire file operation hooks.

    Args:
        event: Event name (e.g. 'file.pre_write').
        file_path: Path to the file being operated on.
        **kwargs: Additional payload fields.
    """
    payload: dict[str, Any] = {"file_path": file_path, **kwargs}
    await dispatch_hook(event, payload)


async def dispatch_shell_hook(event: str, command: str, **kwargs: Any) -> None:
    """Fire shell execution hooks.

    Args:
        event: Event name (e.g. 'shell.pre_execute').
