...
license: MIT
compatibility: designed for bog-agents-cli
---
...
### Skill Location for Bog Agents

The bog-agents CLI loads skills from five sources, listed here from lowest to highest precedence:

| # | Directory | Scope | Notes |
|---|-----------|-------|-------|
| 0 | `<package>/built_in_skills/` | Built-in | Ships with bog-agents CLI |
| 1 | `~/.bog-agents/<agent>/skills/` | User (bog-agents alias) | Default for `bog-agents skills create` |
| 2 | `~/.agents/skills/` | User | Shared across agent tools |
| 3 | `.bog-agents/skills/` | Project (bog-agents alias) | Default for `bog-agents skills create --project` |
...
```
~/.bog-agents/agent/skills/     # user skills (lowest precedence)
├── skill-name-1/
│   └── SKILL.md
└── ...

<project-root>/.bog-agents/skills/   # project skills (higher precedence)
├── skill-name-2/
│   └── SKILL.md
└── ...
```
...
For bog-agents CLI, use any of the skill directories listed in "Skill Location for Bog Agents" above:

```bash
# User skills (default)
scripts/init_skill.py <skill-name> --path ~/.bog-agents/agent/skills

# Project skills
scripts/init_skill.py <skill-name> --path .bog-agents/skills
```
...
#### Option B: `bog-agents skills create` (quick start)
...
bog-agents skills create <skill-name>

# Create in project skills directory
bog-agents skills create <skill-name> --project
```

Use `init_skill.py` when the skill will include bundled resources (`scripts/`, `references/`, `assets/`). Use `bog-agents skills create` for a quick, minimal starting point.
