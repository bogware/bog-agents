...
PERSONA_SYSTEM_PROMPTS: dict[Persona, str] = {
    Persona.ARCHITECT: (
        ...
        "Focus on system design, abstractions, coupling, cohesion, "
        "scalability, and long-term maintainability. Be opinionated "
        "about patterns and anti-patterns. Evaluate whether the "
        "architecture will hold up under growth and team scaling."
    ),
    Persona.SECURITY: (
        "You are a Security Engineer performing a thorough security "
        "review. Focus on OWASP Top 10, input validation, "
        "authentication/authorization, secrets management, dependency "
        "vulnerabilities, injection flaws, and data protection. "
        "Flag any issue that could be exploited."
    ),
    Persona.PERFORMANCE: (
        "You are a Performance Engineer reviewing this codebase. "
        "Focus on algorithmic complexity, memory usage, I/O efficiency, "
        "caching opportunities, database query patterns, and "
        "concurrency. Identify bottlenecks and suggest measurable "
        "improvements."
    ),
    Persona.DX: (
        "You are a Developer Experience lead reviewing this codebase. "
        "Focus on ergonomics, API clarity, error messages, "
        "documentation quality, onboarding friction, IDE support, and "
        "developer productivity. The goal is making this codebase a "
        "joy to work with."
    ),
    Persona.BALANCED: (
        "You are an experienced Staff Engineer performing a "
        "comprehensive review. Balance architecture, security, "
        "performance, and code quality concerns. Prioritize issues by "
...
FOCUS_INSTRUCTIONS: dict[Focus, str] = {
    Focus.ARCHITECTURE: (
        "Analyze the overall architecture: module boundaries, "
        "dependency flow, abstraction layers, coupling between "
        "components, and extensibility points. Evaluate naming "
        "consistency and whether the structure communicates intent."
    ),
    Focus.SECURITY: (
        "Scan for security vulnerabilities: hardcoded secrets, "
        "injection vectors, insecure deserialization, path traversal, "
        "SSRF, broken auth, and dependency CVEs. Check for proper "
        "input validation at system boundaries."
    ),
    Focus.PERFORMANCE: (
        "Profile for performance issues: O(n^2) algorithms, "
        "unnecessary allocations, blocking I/O in async code, missing "
        "caching, N+1 query patterns, and oversized payloads. "
        "Suggest concrete optimizations with expected impact."
    ),
    Focus.TESTING: (
        "Evaluate test coverage and quality: identify untested code "
        "paths, fragile tests, missing edge cases, test isolation "
        "issues, and opportunities for property-based or integration "
        "tests."
    ),
    Focus.CODE_QUALITY: (
        "Review code quality: readability, naming, consistency, dead "
        "code, unnecessary complexity, duplicated logic, type safety, "
        "error handling, and adherence to project conventions."
    ),
    Focus.DOCUMENTATION: (
        "Assess documentation completeness: missing docstrings, "
        "outdated comments, README accuracy, API reference gaps, and "
        "onboarding documentation. Evaluate whether a new developer "
        "could understand the codebase."
    ),
    Focus.DEPENDENCIES: (
        "Review the dependency landscape: outdated packages, unused "
        "dependencies, version pinning strategy, transitive dependency "
        "risks, license compliance, and opportunities to reduce the "
        "dependency tree."
    ),
    Focus.DEPLOYMENT: (
        "Evaluate deployment readiness: CI/CD configuration, "
        "environment variable handling, health checks, graceful "
        "shutdown, logging/observability, container configuration, "
        "and production hardening."
    ),
    Focus.GENERAL: (
        "Perform a general review covering architecture, code quality, "
        "testing, and any issues that stand out. Prioritize findings "
...
class RecommendConfig:
    ...
    Attributes:
        ...
        num_questions: Clarifying questions to ask before reviewing.
...
def build_clarifying_prompt(config: RecommendConfig) -> str:
    ...
    if config.num_questions > 0:
        questions_instruction = (
            f"\n\nBEFORE starting the review, you MUST ask exactly "
            f"{config.num_questions} clarifying questions to better "
            "understand the user's goals and context. These questions "
            "should be specific and actionable — not generic. "
            "Ask about things like:\n"
            "- What are the most critical paths in this codebase?\n"
            "- Are there upcoming changes or deadlines?\n"
            ...
            "Number each question. Wait for the user's answers before "
            "proceeding with the full review."
        )

    return (
        f"{persona_desc}\n\n"
        f"You are performing a review focused on: "
        f"**{config.focus.value.replace('_', ' ')}**.\n\n"
        ...
        f"Include code examples: "
        f"{'yes' if config.include_examples else 'no'}"
        f"{questions_instruction}"
    )


def build_review_prompt(config: RecommendConfig) -> str:
    """Build the full review prompt (after clarifying questions).
    ...
    if config.include_examples:
        example_section = (
            "\n\nFor each finding, include a brief code example "
            "showing the current code and the recommended fix. "
            "Use diff format where appropriate."
        )
    ...
    return (
        ...
        f"**Severity threshold:** Only report "
        f"{config.severity_threshold} and above\n"
        f"**Max findings:** {config.max_findings}\n"
        f"{example_section}\n\n"
        "## Output Format\n\n"
        ...
        "### Recommendations\n"
        "Prioritized list of improvements, grouped by effort "
        "(quick wins vs. larger refactors).\n\n"
        "### Score\n"
        "Rate the reviewed area on a 1-10 scale for: reliability, "
        "maintainability, security, performance, and developer "
        "experience. Include a brief justification for each score."
    )
