...
### The Bog Agents Architecture

The Bog Agents agent harness ships with design patterns validated as good defaults across agentic tasks:
...
uv run harbor run --agent-import-path bog_agents_harbor:BogAgentsWrapper \
...
```txt
Bog Agents → Harbor (evaluate) → LangSmith (analyze) → Improve → Repeat
```
...
python scripts/harbor_langsmith.py create-experiment terminal-bench --name bog-agents-baseline-v1
```
...
python scripts/harbor_langsmith.py add-feedback jobs/terminal-bench/2025-12-02__16-25-40 \
  --project-name bog-agents-baseline-v1
```
