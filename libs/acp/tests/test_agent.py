...
from bog_agents_acp.server import AgentServerACP, AgentSessionContext
...
async def test_acp_agent_prompt_streams_text() -> None:
    ...
    graph = create_agent(model=model, checkpointer=MemorySaver())
...
async def test_acp_bog_agent_hitl_interrupt_on_edit_file_requests_permission() -> None:
    model = GenericFakeChatModel(
        messages=iter(
            [
                AIMessage(
...
async def test_acp_agent_nested_agent_tool_call_returns_final_text() -> None:
    ...
    subagent = create_agent(model=subagent_model, checkpointer=MemorySaver())
    ...
    outer = create_agent(model=outer_model, tools=[call_agent1], checkpointer=MemorySaver())
...
async def test_set_session_mode_resets_agent_with_new_mode() -> None:
    ...
    def agent_factory(context: AgentSessionContext) -> CompiledStateGraph:
        ...
        agent = create_agent(model=model, checkpointer=MemorySaver())
...
async def test_reset_agent_with_compiled_state_graph() -> None:
    ...
    compiled_graph = create_agent(model=model, checkpointer=MemorySaver())
...
async def test_reset_agent_preserves_session_cwd() -> None:
    ...
    def agent_factory(context: AgentSessionContext) -> CompiledStateGraph:
        ...
        return create_agent(model=model, checkpointer=MemorySaver())
