...
This module is the boundary between the filesystem middleware and the optional
video backend (PyAV). It imports PyAV and Pillow lazily so a `bog-agents`
install without the `[video]` extra stays lightweight; the heavy imports only
fire when the agent actually tries to read a video. This mirrors the sibling
`pdf_reader.py` module.

For each video read the module decodes a contiguous slice of the source
(`offset`-seconds skip, `limit`-seconds window) and emits sampled frames at the
configured sampling rate. The output is a list of interleaved text+image content
blocks so the model sees per-frame timestamps alongside the JPEGs.
...
MISSING_VIDEO_HINT = "Reading video files requires the optional video dependencies. Install them with `pip install 'bog-agents[video]'`."
...
def video_dependencies_available() -> bool:
    ...
    discoverable rather than performing a full import. A discoverable but broken
    install (e.g. a compiled extension that fails to load) is reported as
    available here and surfaces later, at actual extraction time, as a
    `VideoExtractionError` carrying `MISSING_VIDEO_HINT`.

    Returns:
        True if both `av` and `PIL.Image` are discoverable, False otherwise.
    """
    try:
...
def _format_timestamp(seconds: float) -> str:
    ...
    Args:
        seconds: Frame offset from the video start in seconds.

    Returns:
        A zero-padded `HH:MM:SS.mmm` string.
    """
    if seconds < 0:
        seconds = 0.0
...
def extract_video_frames(
    ...
    *,
    ...
    Args:
        ...
        sampling_rate: Frames per second to emit. Must be > 0.
    ...
    Raises:
        VideoExtractionError: If PyAV cannot open the payload or the requested
            window yields no decodable frames, or if argument validation fails
...
def _validate_video_window(*, offset_seconds: float, duration_seconds: float, sampling_rate: float) -> None:
    ...
    Raises:
        ValueError: If any of the window parameters is out of range.
    """
    if offset_seconds < 0:
...
def _open_video_container(av: Any, content: bytes) -> Any:
    """Open a video byte payload, normalizing PyAV's failure modes.

    PyAV typically raises `av.error.InvalidDataError` for malformed inputs, but
    it falls back to `OSError` when the system ffmpeg library is missing or
    incompatible. Both surface to callers as `VideoExtractionError` so the
    middleware does not have to distinguish between them.

    Raises:
        VideoExtractionError: If the payload cannot be opened.
    """
    try:
...
def _video_backend_error_types(av: Any) -> tuple[type[BaseException], ...]:
...
def _find_video_stream(container: Any) -> Any:
    """Return the first video stream in `container`.

    Raises:
        VideoExtractionError: If the container has no video stream.
    """
...
def _stream_start_pts(video_stream: Any) -> int:
...
def _stream_start_seconds(video_stream: Any, time_base: float) -> float:
    """Return the stream start timestamp in seconds."""
    return _stream_start_pts(video_stream) * time_base


def _frame_seconds(frame: Any, *, time_base: float, stream_start_seconds: float) -> float | None:
...
def _validate_dimensions(width: int, height: int) -> None:
    ...
    Raises:
        VideoExtractionError: If either side exceeds `MAX_VIDEO_FRAME_SIDE`.
    """
...
def _check_decode_deadline(deadline_seconds: float | None) -> None:
    ...
    Raises:
        VideoExtractionError: If the decode deadline has passed.
    """
...
def _sample_frames_in_window(
    ...
    *,
    ...
    Raises:
        VideoExtractionError: If decoding fails or the first frame alone
...
def _encode_jpeg(frame: Any) -> bytes:
    ...
    Raises:
        VideoExtractionError: If Pillow is not installed or the frame
            dimensions exceed the safety caps.
    """
    try:
