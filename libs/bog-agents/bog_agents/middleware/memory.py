...
```python
from bog_agents import MemoryMiddleware
...
<memory_guidelines>
    ...
    **Examples:**
    ...
    User: Can you write me an example for creating a bog-agents agent in LangChain?
    Agent: Sure, I'll write you an example for creating a bog-agents agent in LangChain <example code in Python>
...
_CLOSE_TAG_RE = re.compile(r"</\s*agent_memory\s*>", re.IGNORECASE)


def _strip_html_comments(text: str) -> str:
    """Remove HTML comments from memory content.
...
def _defang_close_tags(text: str) -> str:
    """Neutralize `</agent_memory>` close-tags inside memory content.

    Args:
        text: Memory text that may contain a forged close-tag.

    Returns:
        Text with every close-tag variant escaped so a memory file cannot forge
        the end of the `<agent_memory>` section of the system prompt.
    """
    return _CLOSE_TAG_RE.sub("<\\/agent_memory>", text)
...
class MemoryMiddleware(AgentMiddleware[MemoryState, ContextT, ResponseT]):
    ...
    See the constructor for the full argument list.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            sources: List of memory file paths to load (e.g., `["~/.bog-agents/AGENTS.md",
                     "./.bog-agents/AGENTS.md"]`).

                     Display names are automatically derived from the paths.

                     Sources are loaded in order.
            add_cache_control: If `True`, tag the last system-message content block
                     with `cache_control: {"type": "ephemeral"}` when the request model
                     is `ChatAnthropic`.
                     ...
                     not qualify.
            system_prompt: System-prompt fragment template. Must contain an
                     `{agent_memory}` slot for runtime memory substitution. Pass `None`
                     to skip appending entirely (memory is still loaded into
                     `state["memory_contents"]`).

        Raises:
            TypeError: If `system_prompt` is not `str` or `None`.
            ValueError: If `system_prompt` is a string missing the `{agent_memory}`
                format slot.
        """
    ...
    def _decode_and_bound(self, path: str, raw: bytes) -> str:
        ...
        return _defang_close_tags(text)
    ...
    def _format_agent_memory(self, contents: dict[str, str], template: str = MEMORY_SYSTEM_PROMPT) -> str:
        ...
        HTML comments are stripped from each source. Stripping happens *after*
        `_decode_and_bound` has defanged close-tags, and the result is defanged
        again, so a comment can neither smuggle nor reassemble a forged
        `</agent_memory>` tag (e.g. `</agent<!--x-->_memory>`).
        ...
        Returns:
            Formatted string with location+content pairs substituted into the
            supplied template.
        """
        if not contents:
        ...
        for path in self.sources:
            ...
            if not raw:
                continue
            stripped = _defang_close_tags(_strip_html_comments(raw)).rstrip()
            if not stripped:
    ...
    def modify_request(self, request: ModelRequest[ContextT]) -> ModelRequest[ContextT]:
        ...
        # Runtime check uses `request.model` (not a flag captured at init) so the
        # breakpoint correctly follows middleware-level model overrides. Runs
        # regardless of `system_prompt` so callers who suppress the fragment still
        # get the prompt-cache breakpoint they asked for.
        if (
