"""Helpers for inspecting and rewriting `create_agent` tool inputs.

Used to apply `HarnessProfile.tool_description_overrides` without mutating
caller-owned tool objects. Ported from the deepagents project for parity.
"""
...
def _tool_name(tool: BaseTool | Callable | dict[str, Any]) -> str | None:
    ...
    Args:
        tool: A tool in any of the forms accepted by `create_agent`.
