...
middleware stacks. The set of *required scaffolding* — classes/names that must
remain in the stack for the agent to function — is owned by `bog_agents.graph`
and threaded through as parameters so policy stays next to `create_agent`.
Ported from the deepagents project for parity.
"""
...
def _validate_excluded_middleware_config(
    ...
    *,
    ...
    prefix on plain names) already fire at `HarnessProfile` construction; this
    function focuses on the assembly-time invariant that scaffolding middleware
    must remain present for the agent to function.
    ...
    if forbidden_classes or forbidden_names:
        ...
        from bog_agents.profiles.harness.harness_profiles import _format_scaffolding_rejection
...
def _raise_on_name_collisions(
    ...
    almost always a surprise — e.g. a user middleware whose `.name` accidentally
    collides with a built-in alias. Force the caller to use a class-form
    exclusion via the runtime `HarnessProfile` to disambiguate.
    """
    ...
    msg = (
        ...
        f"middleware classes within a single stack: {'; '.join(labels)}. Use a "
...
def _apply_excluded_middleware(
    ...
    *,
    ...
    slot-identity semantics of profile merging so a subclass introduced by the
    caller is preserved when the profile excludes the base class. String entries
    match `AgentMiddleware.name` exactly.

    When `matched_classes` / `matched_names` are supplied, matches are recorded
    there so `_verify_excluded_middleware_coverage` can confirm every entry
    matched somewhere across the stacks the profile applies to.
    ...
    Returns:
        A new list with excluded entries removed. Always a fresh list, even when
            no exclusions apply, so callers can mutate the result freely.
    """
    excluded = profile.excluded_middleware
    if not excluded:
        return list(stack)
...
def _verify_excluded_middleware_coverage(
    ...
    *,
    ...
    Run after every stack has been filtered so the accumulated `matched_*` sets
    reflect matches anywhere. An entry that matched nothing is almost always a
    typo or stale profile. Required-scaffolding and `_`-prefixed entries are
    skipped — rejected earlier by `_validate_excluded_middleware_config`.
    ...
    Args:
        ...
        required_classes: Scaffolding classes; subtracted from unmatched so
            scaffolding exclusions don't surface here (defense-in-depth).
        required_names: Scaffolding `.name` values; same purpose.
    ...
    # Private-prefix names are rejected by the config guard; skip them here so
    # the coverage error stays focused on legitimate "didn't match" cases.
    ...
    msg = (
        f"HarnessProfile.excluded_middleware entries matched no middleware across "
        f"any assembled stack: {', '.join(labels)}. Typo or stale profile — every "
        f"exclusion must correspond to a middleware actually present at runtime. "
        f"(Tip: use class-form exclusion when the class is available to catch typos at import time.)"
    )
    raise ValueError(msg)
