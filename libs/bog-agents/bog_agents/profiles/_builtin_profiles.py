...
This bog-agents port ships no built-in provider/harness profiles of its
own — the bootstrap only discovers third-party entry-point plugins. The
public bootstrap surface (`_ensure_builtin_profiles_loaded`,
`_BOOTSTRAP_HARNESS_KEYS`) is preserved so `harness_profiles` and
`provider_profiles` import unchanged.
"""
...
entry-point phases complete — so both bog-agents' own defaults (none, in
this port) and any third-party harness plugins the user has installed are
treated uniformly as "bootstrap-provided." `_has_any_harness_profile`
subtracts this set from the live registry to distinguish those defaults
from profiles the user registers explicitly after import.
"""
...
def _ensure_builtin_profiles_loaded() -> None:
    ...
    1. Register bog-agents' own built-in profiles. This port ships none,
        so the phase is a no-op; the structure is kept so first-party
        defaults can be reintroduced later without changing call sites.
    2. Iterate `importlib.metadata` entry points in the
        `bog_agents.provider_profiles` and `bog_agents.harness_profiles`
        groups. Third-party failures are logged at `WARNING` and skipped
        so one misbehaving distribution cannot prevent `bog_agents.profiles`
        from importing.
    ...
    *during* bootstrap (for example, plugin registration helpers calling
    ...
    try:
        # Phase 1: first-party built-in registrations would go here. This
        # port ships none.
