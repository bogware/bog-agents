...
set. Also pins the OpenRouter upstream provider pool so reasoning-model calls
never route through Azure (which breaks multi-turn reasoning continuity — see
`_OPENROUTER_ALLOW_AZURE_ENV`). Users may layer additional kwargs on top via
...
from bog_agents.profiles.provider.provider_profiles import ProviderProfile, _register_provider_profile_impl
...
_OPENROUTER_APP_URL = "https://github.com/bog-agents/bog-agents"
"""Default `app_url` (maps to `HTTP-Referer`) for OpenRouter attribution.

See https://openrouter.ai/docs/app-attribution for details.
"""

_OPENROUTER_APP_TITLE = "Bog Agents"
"""Default `app_title` (maps to `X-Title`) for OpenRouter attribution."""

_OPENROUTER_ALLOW_AZURE_ENV = "BOG_AGENTS_OPENROUTER_ALLOW_AZURE"
...
_OPENROUTER_ALLOW_AZURE_ENV_LEGACY = "DEEPAGENTS_OPENROUTER_ALLOW_AZURE"
"""Legacy (upstream deepagents) name for the Azure escape hatch.

Still honored for backward compatibility with existing deployments that set
the pre-rename variable. Either name (this or `_OPENROUTER_ALLOW_AZURE_ENV`)
being truthy opts Azure back into the provider pool.
"""

_TRUTHY = {"1", "true", "yes", "on"}
"""Values (case-insensitive) treated as truthy for the Azure escape hatch."""


def _allow_azure() -> bool:
    """Return whether the caller has opted Azure back into the provider pool.

    Reads both the current `BOG_AGENTS_OPENROUTER_ALLOW_AZURE` name and the
    legacy `DEEPAGENTS_OPENROUTER_ALLOW_AZURE` name; either being truthy opts
    in. This keeps pre-rename deployments working after the deepagents port.

    Returns:
        `True` if Azure should be allowed as an OpenRouter upstream provider.
    """
    for env_name in (_OPENROUTER_ALLOW_AZURE_ENV, _OPENROUTER_ALLOW_AZURE_ENV_LEGACY):
        if os.environ.get(env_name, "").strip().lower() in _TRUTHY:
            return True
    return False
...
def _openrouter_attribution_kwargs() -> dict[str, Any]:
    ...
    `BOG_AGENTS_OPENROUTER_ALLOW_AZURE` (or the legacy
...
def check_openrouter_version() -> None:
    ...
    if is_old:
        msg = (
            f"bog-agents requires langchain-openrouter>={OPENROUTER_MIN_VERSION}, "
