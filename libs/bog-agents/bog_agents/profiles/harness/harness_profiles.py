...
!!! beta
    ...
    `bog_agents.profiles` exposes beta APIs that may receive minor changes in
    future releases. Refer to the [versioning documentation](https://docs.langchain.com/oss/python/versioning)
    for more details.

Harness profiles declare how `create_agent` should shape the agent's
...
Users may register profiles via `register_harness_profile`. bog-agents
...
"""Required scaffolding middleware that cannot be excluded by a profile.

In deepagents this set is owned by `deepagents.graph` and imported lazily.
To avoid an import cycle with bog-agents' `graph.py`, the port declares the
canonical names locally. These back filesystem tools and subagent dispatch
and are non-negotiable scaffolding.
"""
...
def _scaffolding_violation_label(entry: object) -> str | None:
    ...
    The required-scaffolding set is declared locally (see
    `_REQUIRED_MIDDLEWARE_NAMES`) to avoid an import cycle with bog-agents'
    `graph.py`, which itself imports this module.
    """
...
class GeneralPurposeSubagentProfile:
    ...
    These settings only affect the default subagent that `create_agent`
...
class HarnessProfileConfig:
    ...
    Example:
        ...
        ```python
        from bog_agents import HarnessProfileConfig, register_harness_profile
    ...
    `create_agent`'s `system_prompt` parameter or
    ...
    !!! note "Removing the `task` tool"
        ...
        `subagents=` on `create_agent`. Async subagents are
        unaffected.
    """
    ...
    subagents via `subagents=` on `create_agent` and the `task` tool
...
class HarnessProfile:
    ...
    and default-subagent adjustments applied by `create_agent` once a
    ...
    Example:
        ...
        ```python
        from bog_agents import HarnessProfile, register_harness_profile
    ...
    See `create_agent`'s `system_prompt` parameter or
    [Prompt assembly](https://docs.langchain.com/oss/deepagents/customization#prompt-assembly)
    ...
    Applied only where bog-agents has a stable description hook: built-in
    ...
    bog-agents middleware from the visible tool set.
    ...
    passed via `create_agent(middleware=[...])`. Merged profiles union
    ...
    !!! warning "Restrictions"
        ...
        - Scaffolding classes (`FilesystemMiddleware`, `SubAgentMiddleware`)
            ...
            `create_agent` resolves the profile. To hide their tools
    ...
    !!! note "Removing the `task` tool"
        ...
        `create_agent`. With nothing to back, the `task` tool is gone.
    ...
    i.e., the stacks that `create_agent` assembles itself.
    ...
    *Not* applied to `CompiledSubAgent` runnables or `AsyncSubAgent` entries.
    ...
    compiled graph with its own middleware chain), so `create_agent` has
    ...
    `create_agent` and the `task` tool is dropped too. Async
...
def _ensure_harness_profiles_loaded() -> None:
    ...
    from bog_agents.profiles._builtin_profiles import _ensure_builtin_profiles_loaded
...
def _has_any_harness_profile() -> bool:
    ...
    registry directly.
    """
    from bog_agents.profiles import _builtin_profiles
...
def _harness_profile_for_model(model: BaseChatModel, spec: str | None) -> HarnessProfile:
    ...
    !!! note "bog-agents port: graceful degradation"

        bog-agents' `_models` exposes `get_model_identifier` but not
        `get_model_provider`. When the `spec` is `None` and those helpers
        are unavailable (or the provider cannot be derived), this function
        falls back to a default `HarnessProfile()`. The `spec is not None`
        fast path always works.
    ...
    # bog-agents' `_models` may not expose `get_model_identifier` /
