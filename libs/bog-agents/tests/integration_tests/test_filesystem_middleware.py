...
class TestFilesystem:
    ...
    def test_longterm_memory_multiple_tools(self):
        ...
        agent = create_agent(backend=backend, checkpointer=checkpointer, store=store)
        assert_longterm_mem_tools(agent, store)

    def test_shortterm_memory_multiple_tools(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        agent = create_agent(backend=StateBackend, checkpointer=checkpointer, store=store)
