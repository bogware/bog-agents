...
Behavioral evaluation suite for the Bog Agents SDK. Test run agents end-to-end against a real LLM and assert on the resulting trajectory (tool calls, final text, file mutations).

## Running

From `libs/bog-agents/`:
...
# Specific model
LANGSMITH_TEST_SUITE=bog-agents-evals uv run --group test --group evals pytest tests/evals --model claude-sonnet-4-6-20250514

# Single test file
LANGSMITH_TEST_SUITE=bog-agents-evals uv run --group test --group evals pytest tests/evals/test_file_operations.py
```

Results are logged to [LangSmith](https://smith.langchain.com/) under the `bog-agents-evals` test suite. Set `--evals-report-file <path>` (or `BOG_AGENTS_EVALS_REPORT_FILE`) to also write a JSON summary.
...
3. Build the agent with `create_agent(model=model, ...)`.
