"""MemoryAgentBench evaluation tests for bog-agents.

Runs the MemoryAgentBench benchmark (ICLR 2026) using the bog-agents runner.
...
def _run_benchmark_sample(
    ...
    """Execute a single MemoryAgentBench sample against bog-agents.
...
def test_conflict_resolution(model: BaseChatModel, config: DatasetConfig) -> None:
    """Evaluate bog-agents on MemoryAgentBench Conflict Resolution tasks.
...
def test_time_learning(model: BaseChatModel, config: DatasetConfig) -> None:
    """Evaluate bog-agents on MemoryAgentBench Test-Time Learning tasks.
