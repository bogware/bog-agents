"""End-to-end unit tests for bog-agents with fake LLM models."""
...
from bog_agents.backends.utils import TOOL_RESULT_TOKEN_LIMIT
from bog_agents.graph import create_agent
from bog_agents.middleware.filesystem import NUM_CHARS_PER_TOKEN
from tests.utils import SampleMiddlewareWithTools, SampleMiddlewareWithToolsAndState, assert_all_agent_qualities
...
class TestBogAgentsEndToEnd:
    """Test suite for end-to-end bog-agents functionality with fake LLM."""

    def test_bog_agent_with_fake_llm_basic(self) -> None:
        """Test basic bog-agents functionality with a fake LLM model.

        This test verifies that a bog-agents can be created and invoked with
        ...
        # Create a bog-agents agent with the fake model
    ...
    def test_bog_agent_with_fake_llm_with_tools(self) -> None:
        """Test bog-agents with tools using a fake LLM model.

        This test verifies that a bog-agents can handle tool calls correctly
        ...
        # Create a bog-agents agent with the fake model and sample_tool
        agent = create_agent(model=model, tools=[sample_tool])
    ...
    def test_bog_agent_with_fake_llm_filesystem_tool(self) -> None:
        """Test bog-agents with filesystem tools using a fake LLM model.

        This test verifies that a bog-agents can use the built-in filesystem
    ...
    def test_bog_agent_with_fake_llm_multiple_tool_calls(self) -> None:
        """Test bog-agents with multiple tool calls using a fake LLM model.

        This test verifies that a bog-agents can handle multiple sequential
    ...
    def test_bog_agent_with_string_model_name(self) -> None:
        """Test that create_agent resolves string model names correctly.
        ...
        with patch("bog_agents.graph.resolve_model", return_value=fake_model):
            agent = create_agent(model="claude-sonnet-4-6", tools=[sample_tool])
    ...
    def test_bog_agent_truncate_lines(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
        ...
        agent = create_agent(model=model, backend=backend)
    ...
    @pytest.mark.parametrize("backend_factory", BACKEND_FACTORIES)
    def test_bog_agent_read_empty_file(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
    ...
    def test_bog_agent_with_system_message(self) -> None:
        ...
        agent = create_agent(model=model, system_prompt=system_msg, middleware=[capturing_middleware])
        assert_all_agent_qualities(agent)
        ...
        assert "You are a Bog Agents agent" in content

    def test_bog_agent_with_system_message_string_content(self) -> None:
        """Test that create_agent accepts a SystemMessage with string content."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
    ...
    def test_bog_agent_two_turns_no_initial_files(self) -> None:
        """Test bog-agents with two conversation turns without specifying files on invoke.
    ...
    def test_bog_agent_two_turns_state_backend_edge_case(self) -> None:
    ...
    async def test_bog_agent_two_turns_no_initial_files_async(self) -> None:
        """Async version: Test bog-agents with two conversation turns without specifying files.
    ...
    async def test_bog_agent_two_turns_state_backend_edge_case_async(self) -> None:
    ...
    @pytest.mark.parametrize("backend_factory", BACKEND_FACTORIES)
    def test_bog_agent_read_file_truncation(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
    ...
    @pytest.mark.parametrize("backend_factory", BACKEND_FACTORIES)
    def test_bog_agent_read_file_no_truncation_small_file(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
    ...
    @pytest.mark.parametrize("backend_factory", BACKEND_FACTORIES)
    def test_bog_agent_read_file_truncation_with_offset(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
    ...
    @pytest.mark.parametrize("backend_factory", BACKEND_FACTORIES)
    async def test_bog_agent_read_file_truncation_async(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
    ...
    @pytest.mark.parametrize("backend_factory", BACKEND_FACTORIES)
    def test_bog_agent_read_file_single_long_line_behavior(self, tmp_path: Path, backend_factory: Callable[[Path], BackendProtocol]) -> None:
...
class TestBogAgentsStructure:
    """Test basic bog-agents agent structure without making network calls."""

    def test_base_bog_agent(self) -> None:
        """Verifies that a basic bog-agents agent can be created with default settings."""
        agent = create_agent()
        assert_all_agent_qualities(agent)

    def test_bog_agent_with_tool(self) -> None:
        """Verifies that a bog-agents agent can be created with tools and the tools are properly bound."""
        agent = create_agent(tools=[sample_tool])
        assert_all_agent_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name

    def test_bog_agent_with_middleware_with_tool(self) -> None:
        """Verifies that middleware can inject tools into a bog-agents agent."""
        agent = create_agent(middleware=[SampleMiddlewareWithTools()])
        assert_all_agent_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name

    def test_bog_agent_with_middleware_with_tool_and_state(self) -> None:
        """Verifies that middleware can inject both tools and extended state channels."""
        agent = create_agent(middleware=[SampleMiddlewareWithToolsAndState()])
        assert_all_agent_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name
