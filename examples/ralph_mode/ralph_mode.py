"""Ralph Mode - Autonomous looping for Bog Agents.
...
Each iteration delegates to `run_non_interactive` from `bog-agents-cli`,
...
async def ralph(
    ...
    *,
    ...
    Each iteration invokes the Bog Agents CLI's `run_non_interactive` with a
    ...
    Args:
        ...
        model_name: Model spec in `provider:model` format (e.g.
            ...
            When `None`, `bog-agents-cli` resolves a default via its config
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Ralph Mode - Autonomous looping for Bog Agents",
...
Examples:
  ...
  python ralph_mode.py "Build an app" --model-params '{"temperature": 0.5}'
    ...
    if args.shell_allow_list:
        from bog_agents_cli.config import parse_shell_allow_list, settings
