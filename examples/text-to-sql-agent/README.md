# Text-to-SQL Bog Agents

A natural language to SQL query agent powered by LangChain's **Bog Agents** framework.  This is an advanced version of a text-to-SQL agent with planning, filesystem, and subagent capabilities.

## What is Bog Agents?

Bog Agents is a sophisticated agent framework built on LangGraph that provides:
...
LANGCHAIN_PROJECT=text2sql-bog-agents
```
...
```python
from agent import create_sql_bog_agents

# Create the agent
agent = create_sql_bog_agents()
...
Bog Agents uses **progressive disclosure** with memory files and skills:
...
## Bog Agents Output Example

The Bog Agents shows its reasoning process:
...
```
text-to-sql-agent/
├── agent.py                      # Core Bog Agents implementation with CLI
...
![Bog Agents LangSmith Trace Example](text-to-sql-langsmith-trace.png)
