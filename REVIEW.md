...
- **#59 Scan jobs + findings ledger** and **#70 Security-scan recipe** *(Shipped 2026-09-06, see §25; kept for the trace.)* (share `findings_store.py`: SQLite keyed by stable fingerprint, triage states, SARIF, CI gate, `--max-cost`; `scan` job kind on `AmbientJob`; `/findings`, `/remediate <id>` → PR with evidence; recipe = architecture map → threat model → hunter subagents on `TaskLedger` → `/jury` → sandbox reproduction).
- **#73 Agent-authored workflows.** *(Shipped 2026-09-06, see §26; kept for the trace.)* `workflow.py` schema (context → work → review → verify → synthesize, each phase a team fan-out under `RunawayCaps`), `author_workflow(description)` tool writing `.bog-agents/workflows/<name>.yaml` loaded as `/name` by `prompt_commands.py`, runner persisting phase state for pause/resume, per-agent meters and a hard per-workflow budget.
- **#69 Plan review screen + `--plan --auto`.** *(Shipped 2026-09-06, see §29; kept for the trace.)* Shared `PlanReviewScreen` (butcher manifests, JTBD specs, plan-mode output; line-addressed comment staging → one revision prompt → re-plan loop), per-slice checkboxes into `ButcherJob`, full-screen execution view on `dashboard.py`, `bog-agents --plan "<prompt>" --auto` in `non_interactive.py`.
- **#75 Memory rebuild + advisor.** *(Shipped 2026-09-06, see §27; kept for the trace.)* `memory_rebuild.py` (pure, injected `invoke`: dedup / contradiction resolution / provenance; candidate store under `.bog-agents/memory.rebuild/`, diff, swap on approval, daemon-schedulable) and an `ask_advisor` tool (one bounded question to the `hard` tier, counted and capped).
- **#76 Team v2.** *(Shipped 2026-09-06, see §28; kept for the trace.)* Typed `Attachment` on `Message` + `send_file`/`receive_files`, SQLite-persisted `Mailbox` (shared with #56), `/add-dir` on `CompositeBackend`, `[worktree] reuse` env cache under `~/.bog-agents/envcache/`, sandbox snapshot templates in `.bog-agents/sandbox.lock`.
...
## 17. Shipped: ROADMAP #48 (2026-09-06, same branch, one commit)

- **Policy is data.** `trust_profiles.py` — `TrustProfile` under `custom_settings.trust` of a `profiles.json` entry (permission mode + `lock_mode`, `restricted`, sandbox level + egress allowlist, allowed/blocked fetch domains, excluded tools). `resolve_trust_profile` picks `--restricted` first, then the named profile, then the empty default; `create_cli_agent(restricted=…)` applies it before *and* after the profile block so a profile cannot re-enable what the flag removed. `trust_controller.mode_refusal` is the one question the App asks before shift+tab / ctrl+t / `/profile` changes the mode.
- **Restricted means "no process", not "no `execute`".** Building the restricted agent showed sixty-odd middleware tools that shell out (PR management, git bisect, preview servers, plugin installs, coverage runners, code intelligence, screenshots, ollama probes). `RESTRICTED_TOOL_NAMES` lists them by family; `_strip_excluded_tools` removes them from every middleware's `tools` — the SDK binds middleware tools to the graph, so filtering the `tools=` argument alone is not enough. `test_no_surviving_restricted_tool_spawns_processes` rebuilds the restricted agent and fails when any surviving tool's module uses `subprocess` or raw sockets, so the list cannot drift silently.
- **Domain policy before DNS.** `web_policy.py` (suffix-matched through the egress proxy's `host_allowed`) is consulted in `assert_fetch_allowed` right after the host is parsed, so a blocked host is never resolved and the check runs offline in tests; `DomainPolicyError` subclasses `WebFetchError`, so redirect handling is unchanged. `web.allowed_domains` / `web.blocked_domains` are manifest options; under `--restricted`, `fetch_url` exists only when some allow-list does.
- **Workspace trust.** `workspace_trust.py` fingerprints the repo-controlled instruction and policy files and stores the record beside the MCP / hooks trust in the same config file; `/permissions trust-workspace` records it and trusts hooks + `.mcp.json` at their current fingerprints in one step, and `/permissions` reports trusted / CHANGED / never acknowledged. The record is *reported*, not enforced — enforcement (prompt on first open, refuse project rules until acknowledged) waits on the posture decision below.
- **Authority tiers.** `authority_file_permissions(restricted=…)` returns a `deny` rule first (`.git/hooks/**`, `.git/config`; restricted adds workflows, editor automation, hooks, expert rules, `.mcp.json`, CLI settings + sandbox config) and the widened `interrupt` rule second; first match wins, so the deny tier cannot be softened by the interrupt tier.
- **Open:** a profile's sandbox level / egress allowlist are parsed but not yet applied to `SandboxConfig.build_local_sandbox`; ACP mode has no `--restricted` (source-only surface); the first-open posture (prompt vs. silent restricted) is the user's call.

## 18. Shipped: ROADMAP #56 (2026-09-06, same branch, one commit)

- **Registry, not a broker.** `bog_agents/session_registry.py` is a directory of JSON records with a heartbeat; liveness is "heartbeat fresh or pid alive" (`pid_alive` opens the process on Windows instead of `os.kill(pid, 0)`, which would *terminate* it there). Writers: the TUI on mount and every 2 s, the daemon per run, all best effort. `find_session` resolves id, name or a unique prefix. `$BOG_AGENTS_SESSIONS_DIR` redirects it (tests, containers).
- **The queue is the existing `Mailbox` on SQLite.** `bog_agents/mailbox_store.py` keeps `send` / `inbox` / `drain` / `@all` semantics (a test runs the same script against both implementations) and adds `wait`, `pending`, `purge`; WAL + `BEGIN IMMEDIATE` give exactly-once `drain` across processes (a subprocess enqueues in the test). `bog-agents queue` sends from a `queue:<id>` sender; the TUI's `_process_next_from_queue` path runs the prompt like typed input, and `turn_finished` (end of `_cleanup_agent_task`) posts the last assistant text back — `--wait` is just a drain on that sender.
- **Detach is ownership transfer, not a new process model.** The server-per-session model already exists; `ServerProcess.detach()` drops the atexit crash hook, this side's log handle and config-dir ownership and forgets the child, so neither `stop()` nor interpreter exit touches it; the registry record turns `detached` with the URL and pid. `ServerProcess.adopt(url, pid)` is the mirror for `bog-agents attach`: a handle that owns no child but can terminate by pid, so quitting an attached session still cleans up. The App changes are thin delegations (`on_mount`, a 2 s interval, the end of `_cleanup_agent_task`, `/detach`, the exit `finally`); `session_controller.py` holds the logic and tests against a fake App. The `/permissions` report body moved into `trust_controller.permissions_report` on the way, which is what kept `app.py` under its ceiling.
- **Daemon drain.** `DaemonScheduler.begin_drain()` refuses new dispatches (skipped runs say why) and `running_count` feeds `/health`; `POST /drain` flips it, SIGTERM and `/shutdown` call it before the existing 30 s wait; a run cancelled by that wait is persisted CANCELLED with a resume hint. `bog-agents daemon drain [--timeout] [--stop]` polls `/health`; `daemon upgrade` chains drain → stop → package upgrade → start.
- **Open:** the detached server dies with the console window on Windows (it is still the TUI's `langgraph dev` child); hosting it under the daemon service is the broker follow-up. `serve` does not register yet. `LangGraph RunControl.request_drain()` is not in 1.2.9, so drain waits for turns rather than checkpointing mid-turn.

## 19. Shipped: ROADMAP #53 (2026-09-06, same branch, one commit)

- **Objective is a shift, not a second judge.** `apply_objective(tier, objective, blocked)` moves the judged tier one rung down (`cost`) or up (`intelligence`); the judge prompt is unchanged, so the decision log keeps both tiers and the counterfactual stays honest. `blocked` comes from the log: three or more downgrades of a tier ruled `bad` (`/operator verdict bad`) with a bad ratio of at least a half stop that tier being downgraded — deterministic, user-driven learning, no model in the loop.
- **Decisions are data.** `operator_decisions.py` appends every routing decision (judged / routed tier, both models, effort, route, preview) and `operator_turn_finished` (end of `_cleanup_agent_task`) writes the turn's token delta and cost back onto it; session stats may be cumulative or per-turn, so the delta falls back to the absolute value when the counter went backwards. `counterfactual()` prices the same tokens on the judged tier's model through the SDK catalog (local models cost 0), which is the `/cost` line.
- **Failover is vendor-neutral.** `ProviderFailoverMiddleware` classifies by status code (429 / 529 / 503 / 402), exception class name (`RateLimitError`, `ResourceExhausted`, …) and message substrings, parks the primary using any reset header the provider sent (`retry-after` seconds or HTTP date, OpenAI `6m0s` durations, Anthropic RFC 3339 resets, epoch `x-ratelimit-reset`, capped at six hours) or the cooldown, sticks to the first working fallback while parked, and tries the primary again when the park expires. Unrelated exceptions propagate untouched; all-fallbacks-fail re-raises the original. `build_model` and the clock are injected, so the tests run without a provider.
- **Wiring.** `create_cli_agent` attaches the middleware when `[models].fallbacks` is set and the model is not Bedrock; `_configured_fallbacks` / `_build_fallback_model` are module functions so tests can stub them. `[pool.judge]` feeds `_judge_model_spec` ahead of the easy tier.
- **Open:** the parked state has no status-bar row yet (the `[failover]` note in the first answer after a hop is the surface); pool entries other than `judge` are parsed and exposed but not yet consumed by subagents / summarization / butcher workers.

## 20. Shipped: ROADMAP #64 (2026-09-06, same branch, one commit)

- **Result replacement is a MODIFY, not an OBSERVE.** `PostToolUse` moved from OBSERVE to MODIFY so the type table stays honest: `parse_hook_decision` returns `tool_result` for `{"tool_result": …}` and the middleware's `_after` swaps the `ToolMessage` content (or marks it an error on `block`) before the model sees it. The async path runs the post hooks off the loop like the pre hooks.
- **Failure policy is per hook.** `_run_decision_hook` now reports `failed=True` instead of a silent allow; `_on_failure` maps it through `on_failure: allow|deny|ask`. `ask` is the interesting one: the server-side middleware cannot prompt, so `approval_hook_verdicts` (called from the approval adapter before the auto-approve decision) re-runs only the `ask` hooks and, when one fails, forces the prompt for the batch; the middleware then treats `ask` as allow because a human already looked. `PermissionRequest` hooks run in the same place and reject with the hook's reason.
- **Trust-by-hash.** `script_sha256` hashes the script a command runs (never the interpreter — a missing script must not pin `python.exe`); `pin_hook_hashes` stamps `sha256` + `base`; `_hash_refusal` denies with "changed since it was trusted". `trust_plugin` records `hook_hashes` for the plugin's `hooks/*.json` scripts and `load_plugin_hooks` skips a plugin whose current hashes differ (warning names the plugin), with relative script paths resolved against the plugin root so the hooks run from any cwd.
- **Prompt hooks are fail-closed by construction.** `evaluate_prompt_hooks` denies on judge exceptions, out-of-shape answers and a missing judge; `build_prompt_invoke` runs `invoke_model` in its own loop on a worker thread so the same callable serves `wrap_tool_call` and `awrap_tool_call`. The CLI hands it the agent's own model.
- **`/model` and Escape.** `model_switch_refusal` gates `_switch_model` (deny → error message, no switch); `announce_model_switch` fires `PostModelSwitch`; `action_interrupt` fires `Interrupt` before cancelling the worker. app.py stayed under its ceiling by hoisting `dispatch_hook_fire_and_forget` to the module imports.
- **Open:** `ask` for hooks that only exist server-side (no approval UI, e.g. the daemon) degrades to allow with a log line; Claude Code's `hookSpecificOutput.updatedInput` (PreToolUse argument rewriting from vendor files) is still only honoured through the project-hooks `modify` path.

## 21. Shipped: ROADMAP #71 (2026-09-06, same branch, one commit)

- **Fork is a seeding rule, not a new runtime.** `seed_fork_messages(parent_messages, task)` is the whole mechanism: drop the parent's `SystemMessage`s (the child has its own), drop the trailing `AIMessage` whose `task` call is being served (it has no `ToolMessage` yet — leaving it would hand the child an unbalanced history), append the task. `_build_task_tool` looks the mode up on the raw spec, so `mode: "fork"` works for user subagents and for the built-in one alike; isolated children are untouched (a test pins that they see only the task).
- **The built-in `fork` subagent carries the parent's prompt.** graph.py builds it from the general-purpose spec with `_fork_system_prompt` (prefix, base, suffix, profile suffix — the same parts the parent assembles) so the base prompt and tool schemas match the parent's; `FeatureConfig.enable_fork_subagent=False` removes it. The middleware-appended sections still differ (the child has no `SubAgentMiddleware`), which is why the prefix hit is partial — recorded as open.
- **CLI forks are background agents with a brief.** `fork_controller.conversation_brief` renders the last user / assistant turns from the message store; `/subtask` submits brief + prompt to `BackgroundAgentManager` with `parent_thread_id`; `/fork` first records `session_fork.create_fork` (so `/threads` and the tasks tree can relate them) and then either submits in place or hands off to `/agent spawn --worktree`. One App handler serves both commands; the `/threads search` branch moved into `thread_flags.maybe_run_threads_verb` to pay for it under the app.py ceiling.
- **Parity leg.** `.github/workflows/ci.yml` already runs `test_deepagents_compat.py` + `test_graph_upstream_parity.py` against `--with deepagents` (latest) on every PR; no change was needed, the plan's "pin to latest" was already the behaviour.
- **Open:** `grep_max_count` on the backend protocol (a signature change across every backend); fork mode for `run_team` / butcher workers; copying the checkpoint thread itself into the fork (today the brief travels, the thread id is recorded).

## 22. Shipped: ROADMAP #74 (2026-09-06, same branch, one commit)

- **The chain is the artefact.** `action_log.py` keeps one JSONL per run where every event's `hash` covers its predecessor's hash and its own canonical body; `verify_chain` reports the first break with a reason (edited / removed / reordered / unparseable) and `verify_export` re-checks an exported bundle plus its detached signature. Signing is injected (`sign(payload) -> str`), so the SDK stays dependency-free and the CLI hands in its TraceFile Ed25519 key; the bundle's `signer` / `signature` are excluded from the signed bytes (`signed_payload`) so verification is unambiguous. Two chains per session by design — the agent's (server process, `ActionLogMiddleware`) and the approvals / verdicts chain (TUI process) — because two writers on one file would interleave and break the hash order.
- **Spans without the OTel SDK.** `otel_export.py` builds OTLP/HTTP JSON by hand (`resourceSpans` → `scopeSpans` → spans with typed attributes) and posts it with `urllib`; the attribute names follow the GenAI semantic conventions (`gen_ai.operation.name` = `chat` / `execute_tool` / `invoke_agent`, `gen_ai.request.model`, `gen_ai.usage.input_tokens` / `output_tokens`, `gen_ai.tool.name`, `gen_ai.agent.name`) plus `bog.cost_usd` from the SDK price catalog. A sink is a one-method protocol, so tests use `BufferSink` and a LangSmith adapter is a small follow-up rather than a rewrite.
- **Usage export equals the ledger.** `SpendLedger.records()` is the only new SDK API; the daemon rolls it up per `(day, scope, model)`, writes CSV or OTLP metrics (`bog.usage.*` monotonic sums with `bog.scope` / `bog.kind` / `bog.owner` / `gen_ai.request.model` attributes), and the tests assert the CSV sums equal `totals_by_scope` for the same day. `GET /usage` and `POST /usage/export` sit behind the daemon token; `bog-agents daemon usage-export` fronts them.
- **Wiring.** `FeatureConfig.enable_action_log` / `action_log_dir` / `action_log_run_id` / `otel_endpoint` / `otel_headers` → graph.py appends the two middleware after the audit trail; `create_cli_agent` fills them from `compliance.*`; `record_approval_decisions` mirrors every decision; the Expert controller passes `expert_audit_sink()` as the engine's `audit`. `/threads` gained typed `delete <id>` / `resume <id>` verbs on the way (the honesty audit stopped seeing them once `search` moved out of the App handler).
- **Open:** collector auth beyond static headers; per-user attribution for daemon usage (scopes name jobs); a LangSmith `SpanSink`; a `/actionlog` twin for the daemon's run chains (they are written, not yet browsable from the CLI).

## 23. Shipped: ROADMAP #72 (2026-09-06, same branch, one commit)

- **The bridge is the feature.** The child interpreter has no tools; `tools.<name>` is a proxy that writes `{"op": "call", ...}` on stdout and blocks on stdin for the answer. The parent serves each call by building a `ToolCallRequest` and pushing it through the same composed `wrap_tool_call` chain the model's direct calls use (`_compose` mirrors langchain's tool node, outermost first), minus `HumanInTheLoopMiddleware` and code mode itself. A middleware that denies (the Expert engine, SafeTools, a `PreToolUse` hook) returns an error `ToolMessage`, which the child raises as `ToolDenied` — the test's stand-in policy proves the tool body never runs.
- **Deliberately stricter than a direct call.** Interrupts cannot cross the subprocess boundary, so any tool in the session's `interrupt_on` map is refused inside a script with a message that says to call it directly; `task` / `web_search` go through `CostLedger.register_*` before execution; a `max_calls` budget and a killing timer bound every script. `python -I -u` with a scrubbed environment and a throwaway cwd is the floor; `LocalSandbox` wraps it where bubblewrap / seatbelt exist, and the Windows gap is named in the log.
- **Protocol lessons.** Only `call` messages wait for a reply — `print`, `error` and `done` are one-way, otherwise the child deadlocks on the first print. The parent reads with `readline()` rather than iterating the pipe.
- **Wiring.** `FeatureConfig.enable_code_mode` appends the middleware; graph.py binds it right before `notify_assembly` with the explicit tools, the middleware list and the merged `interrupt_on` names (HITL is created inside `_langchain_create_agent`, so the gate list has to be passed, not discovered). The CLI's `tools.code_mode` option feeds it and `--restricted` strips `run_code` / `execute_mcp_script`.
- **Open:** QuickJS as a second interpreter (the runner protocol is language-neutral — a JS shim would speak the same lines); `task()` with a response schema; running the child under `LocalSandbox` on Windows once #60 lands.

## 24. Shipped: ROADMAP #50 (2026-09-06, same branch, one commit)

- **One document, one verifier.** `verify_document` checks an Ed25519 signature over the canonical `policy` bytes with the pinned public key (the TraceFile `KeyMaterial` / `verify` pair, no new crypto); `sign_document` is the org-side twin used by the tests, which sign with a fresh key, tamper the body and re-sign with a different key to prove both rejections. A URL source without a pinned key is refused outright; a path source may be unsigned (a repo-committed policy is at least reviewable), and the row says `UNSIGNED` so nobody mistakes it.
- **Cache is for outages, not for stale policy.** `load_managed_policy` writes the last good document beside the config and reads it only when the fetch fails or overflows; `active_policy()` loads once per process (an hour's memo), `refresh=True` re-fetches (`/doctor` uses it).
- **Enforcement points are one-liners on purpose.** Each site imports `active_policy()` lazily inside a `try` and applies one pure method: `filter_mcp_servers` after `merge_mcp_configs`, `locked_base_url` after the config.toml `base_url`, `plugin_verdict` before `InstallResult` (the directory is removed again), `model_switch_refusal` ahead of the `PreModelSwitch` hooks, `skill_allowed` through the SDK's new `set_skill_dir_filter` (which runs after the symlink containment check on both listing paths). A policy that fails to load never breaks a session — it is logged and the row in `/permissions` / `/doctor` says so.
- **Facts for rules.** Every `/model` attempt asserts a `model_switch{model, refused, reason}` fact into the Expert engine (same best-effort pattern as approval decisions), so a YAML rule can react to switches the policy allowed as well as the ones it refused.
- **Open:** how orgs distribute the public key (env var / config today); `zero_retention` should also stop the sidechain writer (memory is off; sidechains still append); a `/policy` command (`show`, `reload`) — the rows live under `/permissions` for now.

## 25. Shipped: ROADMAP #59 + #70 (2026-09-06, same branch, one commit)

- **The fingerprint is the feature.** `fingerprint(rule_id, path, message)` hashes the rule, the slash-normalised path and a whitespace-collapsed, number-free, lower-cased message — never the line number — so a finding survives a line shift, `occurrences` counts re-sightings, and a triage decision (`wontfix`, `false_positive`, with its note) stays attached until the code changes enough to change the message. `record()` returns `(new, updated, reopened)`; `resolve_missing(source, run_id)` closes what one source stopped reporting and never touches another source's rows.
- **One line format, one constant.** Every producer (daemon scan prompt, `security-scan` recipe, `/findings record` of any report) asks the model for `<path>:<line> [<severity>] <RULE_ID>: <message>` under `## Findings`; `parse_findings_text` tolerates bullets, numbering, `path:line:col`, a severity word in the message and no rule (defaults to `finding`), and ignores prose so a review's narrative never becomes rows.
- **Daemon scan jobs are ordinary jobs.** `scan_profile` swaps the prompt source; the output path is unchanged (`run.output` gains the ledger summary, `run.error` carries a failed gate while the status stays `COMPLETED`, so dispatch targets still fire and `/runs` is honest). The ledger lives beside the scanned repo, not in the daemon dir, so the interactive `/findings` in that repo sees the nightly scan's rows.
- **CI reads one of two doors.** `bog-agents command "/findings gate --max high"` exits 1 when the gate fails (`HeadlessResult.ok`), and `GET /findings/gate` on the daemon returns `passed`; both use `FindingsStore.gate`. SARIF is `partialFingerprints["bogFingerprint/v1"]` so code-scanning dedups on the same key.
- **Room in `app.py`.** `/jury`'s diff gathering moved to `jury.gather_diff` (pure, argv-form git, 15 s cap); app.py went from 17,895 to 17,866 lines with the two new handlers included.
- **Open:** `/remediate` prepares the fix turn (and marks the row `triaged`) but does not open the PR — `/pr --evidence` does; sandbox reproduction is a recipe step whose verdict lives in the report text, not a ledger column; hunters run inside one turn rather than as a `TaskLedger` fan-out; there is no `/findings prune` (rows are small; `fixed` rows stay for history).

## 26. Shipped: ROADMAP #73 (2026-09-06, same branch, one commit)

- **A workflow is a team per phase.** Each phase builds a `TaskLedger` from its tasks (with `depends_on` inside the phase) and runs `bog_agents.teams.run_team` with `workers` members, so dependency-aware claims, the spawn cap and the cost cap are the same code `/team run` uses — the runner only adds phase order, gates and meters. The model side is one injected `WorkflowTaskRunner`; the real one (`workflow_controller.build_task_runner`) is a non-interactive, auto-approving `create_cli_agent` per task with tokens summed from `usage_metadata` and cost from the SDK price catalog.
- **Resume is state, not memory.** `WorkflowRun` (phase status, per-task `TaskMeter`) is saved after every task; `run_workflow(run=…)` skips `done` phases and rebuilds a paused phase's ledger from its unfinished tasks only. A run that already spent its budget is refused until `--budget` raises it, so a resume never silently overspends.
- **Gates are text contracts.** Review/verify tasks are told to end with `VERDICT: PASS|FAIL`; `_verdict` reads the tail of the output (a runner may set `passed` explicitly). One failing verdict fails the phase and stops the run at that phase with the reason in `stop_reason`.
- **Authoring is the agent's job, validation is ours.** `/workflow author` (model call) and the `author_workflow` tool both go through `parse_workflow`; the tool returns the schema with the error so the agent fixes its own YAML. Tools only appear when the project already has `.bog-agents/workflows/` (or `tools.workflows`), so the default agent's schema budget is unchanged.
- **Open:** runs are not shown in the `/tasks` tree (they run as tracked sessions, so Esc still stops them); the code-flavoured `workflow` tool variant is not built; `workers` beyond one share the repository working tree like `/team run` does (no per-worker worktrees).

## 27. Shipped: ROADMAP #75 (2026-09-06, same branch, one commit)

- **Candidate, diff, approve.** `rebuild()` never touches the live store: it writes `memory.md`, `candidate.diff` and `report.json` under `.bog-agents/memory.rebuild/`; `apply_candidate()` backs the live file up beside them and swaps atomically. The managed section is the unit — text before and after it (hand-written notes, other headings) passes through untouched, so a rebuild cannot eat a human's paragraph.
- **The model is optional.** `consolidate(invoke=None)` is a deterministic whitespace-insensitive dedup; with a model, the reply must be JSON `{"entries": [{text, category, sources}], "notes"}` and anything unusable falls back to that dedup with a note saying so. `--dedup` and the headless twin never call a model, which is what makes it safe to schedule.
- **Provenance is per entry.** The prompt numbers the current entries and labels transcripts `thread:<id>`; each rebuilt entry carries the `sources` it cites, kept in `report.json` (the store line stays the plain `- (category) fact` the `remember` tool writes).
- **Advisor as a tool, not a route.** `ask_advisor` is a `StructuredTool` with an injected `ask(system, prompt)`; the bundle bounds sizes, counts questions, reports the cap in-band ("decide with what you have and say so") and charges tokens through `on_usage` into `cost_ledger.tracker_for("advisor")`. `hard_tier_model` resolves the operator preset's `hard` tier and returns `None` when the active model already is it, so the tool never asks a model to advise itself.
- **Open:** transcripts come from the latest checkpoint per thread (tool messages skipped, last 6k chars) rather than the FTS index; the rebuild has no daemon job preset; the advisor's answers are not yet surfaced in `/cost` beyond the ledger row.

## 28. Shipped: ROADMAP #76 (2026-09-06, same branch, one commit)

- **Attachments are content-addressed copies, not paths.** `send_file` stages a copy under the exchange directory (text through the DLP patterns first, the redaction count on the record; directories zipped with `.git` / `node_modules` / `.venv` skipped; `send_patch` is `git diff HEAD --binary` plus creation hunks for untracked text files), so a teammate receiving a file cannot be handed a path outside the tree and cannot see a secret the sender's tree held. `receive_files` peeks the inbox (never `drain`s, so the claim loop's "Team notes" still arrive) and delivers each sha256 once.
- **One `Message`, two stores.** `Attachment` lives on `teams.Message`; `MailboxStore` serialises it into a JSON column added with an in-place `ALTER TABLE` for stores created before #76, and both stores share the same `send(..., attachments=)` signature so `team_file_tools` is store-agnostic. `/team run` now gets a `MailboxStore` keyed by the interactive thread, which is what lets a `/tasks steer` from a later session reach a teammate.
- **Env reuse is keyed on the lockfile, never on the branch.** `envcache.plan_reuse` refuses to link when the worktree's lockfile hash differs from the cache key, seeds the cache from the main checkout only when its hash matches, and a link failure degrades to "install normally" rather than failing the worktree. Junctions on Windows (no privilege needed), symlinks elsewhere.
- **Mounts are declared, then built.** `/add-dir` writes `.bog-agents/mounts.json`; `create_cli_agent` turns each into a `FilesystemBackend` route at `/mnt/<name>/`. The backend graph is built once per server, so the command says plainly that the mount applies on the next start rather than pretending.
- **Open:** `sandbox.lock` is recorded and validated but no provider reads it yet; `/add-dir` has no live rebuild; attachments have size caps rather than streaming; there is no `/team files` view of the exchange directory.

## 29. Shipped: ROADMAP #69 (2026-09-06, same branch, one commit)

- **Review is addressed to lines, not to a blob.** `PlanReview.comment(n, text)` and `toggle(slice)` are the only two verbs; `revision_prompt()` quotes each comment against the line it targets so the planner changes exactly what was questioned, and `execution_brief()` carries deselected slices as `[SKIPPED by reviewer]` plus the comments as constraints. One model serves butcher manifests (rendered as `## Slice N` blocks), JTBD specs, plan-mode output and any plan file.
- **The screen owns no model calls.** `PlanReviewScreen` returns a `PlanReviewResult`; `plan_review_controller.decide` turns it into a prompt (approve → brief, revise → revision prompt, cancel → nothing) and, for a butcher job, writes the slice selection into `manifest.json`. That split is what lets the screen be driven by a Textual `run_test()` pilot in the unit suite.
